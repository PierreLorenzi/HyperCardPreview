...
extension Layer {
    ...
    func initLayerProperties(layerReader: LayerBlockReader, version: FileVersion, layerType: LayerType, loadBitmap: @escaping (Int) -> MaskedImage, styles: [IndexedStyle]) -> Property<ContentMap> {
        ...
        let contentsProperty = Property<ContentMap> { () -> ContentMap in
            return Layer.mapContents2(layerReader: layerReader, version: version)
        }
    ...
    private static func mapContents2(layerReader: LayerBlockReader, version: FileVersion) -> ContentMap {
        ...
        let cardContentMap = Layer.index2(cardContents, by: { $0.readIdentifier() })
        let backgroundContentMap = Layer.index2(backgroundContents, by: { $0.readIdentifier() })
        
        return ContentMap(cardContents: cardContentMap, backgroundContents: backgroundContentMap)
    }
    
    static func index2<T, Index: Hashable>(_ elements: [T], by closure: (T) -> Index) -> [Index: T] {
        var map: [Index: T] = [:]
    ...
    static func loadParts(layerReader: LayerBlockReader, layerType: LayerType, contentsProperty: Property<ContentMap>, styles: [IndexedStyle]) -> [LayerPart] {
        ...
        for partBlock in partBlocks {
            ...
            case .button:
                let loadContent = { () -> HString in
                    return Layer.loadContent2(identifier: partReader.readIdentifier(), layerType: layerType, contentsProperty: contentsProperty, styles: styles).string
                }
                let button = Button(loadFromData: partBlock, loadContent: loadContent)
                parts.append(LayerPart.button(button))
            case .field:
                let loadContent = { () -> PartContent in
                    return Layer.loadContent2(identifier: partReader.readIdentifier(), layerType: layerType, contentsProperty: contentsProperty, styles: styles)
                }
                let field = Field(loadFromData: partBlock, loadContent: loadContent)
                parts.append(LayerPart.field(field))
            }
    ...
    static func loadContent2(identifier: Int, layerType: LayerType, contentsProperty: Property<ContentMap>, styles: [IndexedStyle]) -> PartContent {
        ...
        return loadContentFromReader2(contentReader: contentReader, styles: styles)
    }
    
    static func loadContentFromReader2(contentReader: ContentBlockReader, styles: [IndexedStyle]) -> PartContent {
